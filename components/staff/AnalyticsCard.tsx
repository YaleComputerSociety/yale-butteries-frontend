...
const AnalyticsCard: FC<Props> = ({ time, name, numItems, cost, items }: Props) => {
  ...
  return (
    <View style={styles.backgroundCard}>
      ...
      >
        ...
        <Text style={styles.text}>{numItems.toString()}</Text>
        <Text style={styles.text}>${cost}</Text>
      </View>

      {items?.map((item, j) => (
        <AnalyticsItemCard key={j} hide={!isExpanded} name={item.name} cost={(item.price / 100).toFixed(2)} />
      ))}
    </View>
  )
}
